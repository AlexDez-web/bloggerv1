...
if ($is_logged && isset($_GET['user']) && !empty($_GET['user']) && intval($_SESSION['id']) !== intval($_GET['user'])) {
...
$query_articles = "SELECT * FROM articles where id_user = '$id_user'";
                    ...
                    <div class="container-blog-child">
                        <div class="profil-info">
                            <div class="name-initial">
                                <?php echo $fetch_user['nom'][0] . $fetch_user['prenom'][0]; ?>
                            </div>

                            <div class="profil-name"><?php echo $fetch_user['nom'] . " " . $fetch_user['prenom']; ?>
                            </div>
                            <div class="modifier-text">
                                <a href="modifier-profile.php">
                                    <?php
                                    if ($is_logged) {
                                        if (!(isset($_GET['user']) && !empty($_GET['user']) && intval($_SESSION['id']) !== intval($_GET['user']))) {
                                            echo "Modifier mes informations";
                                        }
                                    }
                                    ?>
                                </a>
                        ...
                        <div style="margin-top: 1rem;text-align: center;">
                            <h1><?php
                                if ($is_logged) {
                                    if (isset($_GET['user']) && !empty($_GET['user']) && intval($_SESSION['id']) !== intval($_GET['user'])) {
                                        echo "Articles plubié par " . $fetch_user['nom'] . " " . $fetch_user['prenom'];
                                    } else {
                                        echo "Mes articles plubié";
                                    }
                                } else {
                                    echo "Articles plubié par " . $fetch_user['nom'] . " " . $fetch_user['prenom'];
                                }
